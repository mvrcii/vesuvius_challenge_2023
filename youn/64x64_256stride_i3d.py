...
def read_image_mask(fragment_id, start_idx=15, end_idx=45):
    ...
    for i in idxs:
        ...
        path = os.path.join("..", "data", "fragments", f"fragment{fragment_id}", "slices", f"{i:05}.tif")
        print("Loading", path)
    ...
    label_file_path = os.path.join("..", "data", "youn_labels", f"{fragment_id}_inklabels.png")
    label_file = cv2.imread(label_file_path, 0)

    fragment_mask_path = os.path.join("..", "data", "fragments", f"fragment{fragment_id}", "mask.png")
    ...
    if 'frag' in fragment_id:
        ...
        label_file = cv2.resize(label_file, (label_file.shape[1] // 2, label_file.shape[0] // 2),
                                interpolation=cv2.INTER_AREA)

    label_file = label_file.astype('float32')
    label_file /= 255
    return images, label_file, fragment_mask
...
def main():
    ...
    CFG.valid_id = val_frag_id

    val_frag_label_path = os.path.join("..", "data", "youn_labels", f"{val_frag_id}_inklabels.png")
    valid_mask_gt = cv2.imread(val_frag_label_path, 0)
    ...
    trainer = pl.Trainer(
        ...
        callbacks=[ModelCheckpoint(filename=f'valid_{val_frag_id}_{fold}_fr_{enc}' + '{epoch}', dirpath=CFG.model_dir,
                                   monitor='val/total_loss', mode='min', save_top_k=1),
