...
def read_fragment(fragment_id, offset, single_image=False):
    ...
    if single_image:
        print("just extracting slice", end)
        idxs = [end]
    else:
...
def infer_full_fragment(model, images):
...
if __name__ == '__main__':
    ...
    for offset in range(-offset_magnitude, offset_magnitude + 1):
        ...
        logit_path = os.path.join(results_dir, f"logits_fragment{fragment_num}_offset_{offset}.png")
        plt.savefig(logit_path, bbox_inches='tight', dpi=500, pad_inches=0)

        # save binary
        plt.imshow(result > 0.5, cmap='gray')
        binary_bath = os.path.join(results_dir, f"binarized_fragment{fragment_num}_offset_{offset}.png")
