...
def read_image(fragment_id):
    ...
    for i in tqdm(idxs):
        img_path = os.path.join(r"A:\projects_a\Python\vesuv\data", "fragments", f"fragment{fragment_id}", "slices",
                                f"{i:05}.tif")
    ...
    label_path = os.path.join(r"A:\projects_a\Python\vesuv\data", "fragments/fragment2/inklabels.png")
    ...
    for y1 in tqdm(y1_list):
        for x1 in x1_list:
            ...
            if images[y1:y2, x1:x2].max() != 0:
                ...
                label_patch = label[y1:y2, x1:x2]
                label_patch = resize(label_patch, (128, 128), order=0, preserve_range=True, anti_aliasing=False)
