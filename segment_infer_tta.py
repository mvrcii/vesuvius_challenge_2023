...
def read_fragment():
    ...
    for i in tqdm(idxs):
        img_path = os.path.join(CFG.fragment_root_dir, "segments", "scroll1recto", "slices", f"{i:05}.tif")
...
if __name__ == '__main__':
    ...
    # inference
    result = infer_full_fragment(checkpoint_path)

    date_time_string = datetime.now().strftime("%Y%m%d-%H%M%S")
    results_dir = os.path.join("inference", "results", "scroll1", date_time_string)
    os.makedirs(results_dir, exist_ok=True)

    # save logits
    plt.imshow(result, cmap='gray')
    logit_path = os.path.join(results_dir, f"logits_scroll1_{date_time_string}.png")
    plt.savefig(logit_path, bbox_inches='tight', dpi=500, pad_inches=0)

    # save binary
    plt.imshow(result > 0.5, cmap='gray')
    binary_bath = os.path.join(results_dir, f"binarized_scroll1_{date_time_string}.png")
    plt.savefig(binary_bath, bbox_inches='tight', dpi=1500, pad_inches=0)

    print("Saved results to", results_dir)
    # save raw output
    np.save(os.path.join(results_dir, f"scroll1_{date_time_string}result.npy"), result)
